...
M = [[str(string.uppercase[c] + str(r + 1), 'utf8')
      for c in DEFAULT_RANGE] for r in DEFAULT_RANGE]
MT = [[str(string.uppercase[c] + str(r + 1), 'utf8')
...
class TestIxl(unittest.TestCase):
    ...
    def test_read_sheet(self):
        ...
        def scan_ranges(i, f, s):
            ...
            self.assertEqual([m[i:f] for m in M],
                             self.xlrw.read_sheet(s, c0=i, cf=f))
            self.xlrw.by_row = False
            self.assertEqual(MT[i:f], self.xlrw.read_sheet(s, c0=i, cf=f))
            self.assertEqual([m[i:f] for m in MT],
                             self.xlrw.read_sheet(s, r0=i, rf=f))
    ...
    def test_read_book(self):
        for book in TEST_BOOKS:
            ...
            self.assertEqual(SUB_M, self.xlrw.read_book(book,
                                         0, r0=5, rf=20, c0=3, cf=15))

    def test_guess_read(self):
        for book in TEST_BOOKS:
            self.assertEqual(M, self.xlrw.guess_read(
